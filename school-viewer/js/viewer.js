window.addEventListener('DOMContentLoaded', () => {
  ...
  engine.runRenderLoop(() => {
    if (characterMesh) {
      ...
      if (moving) {
        if (keysPressed["q"]) characterMesh.moveWithCollisions(new BABYLON.Vector3(-speed, 0, 0)); // ← 左
        if (keysPressed["c"]) characterMesh.moveWithCollisions(new BABYLON.Vector3(speed, 0, 0));  // → 右
        if (keysPressed["e"]) characterMesh.moveWithCollisions(new BABYLON.Vector3(0, 0, -speed)); // ↑ 前
        if (keysPressed["s"]) characterMesh.moveWithCollisions(new BABYLON.Vector3(0, 0, speed));  // ↓ 後
